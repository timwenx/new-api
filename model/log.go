...
func RecordErrorLog(c *gin.Context, userId int, channelId int, modelName string, tokenName string, content string, tokenId int, useTimeSeconds int,
	...
	// 未设置时默认记录 IP；用户显式关闭时才不记录。
	needRecordIp := true
	if settingMap, err := GetUserSetting(userId, false); err == nil {
		needRecordIp = settingMap.ShouldRecordIpLog()
	}
	log := &Log{
		UserId:           userId,
		Username:         username,
...
func RecordConsumeLog(c *gin.Context, userId int, params RecordConsumeLogParams) {
	...
	countedTokens, scaleErr := common.ScaleTokenCount(
		int64(params.PromptTokens)+int64(params.CompletionTokens),
		tokenMultiplier,
		MaxWeeklyTokenLimit,
	)
	if scaleErr != nil {
		common.SysError("failed to apply log token multiplier: " + scaleErr.Error())
		tokenMultiplier = 1
		countedTokens = int64(params.PromptTokens) + int64(params.CompletionTokens)
	}
...
func SumUsedQuota(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, group string) (stat Stat, err error) {
	...
	// 为rpm和tpm创建单独的查询
	rpmTpmQuery := LOG_DB.Table("logs").Select("count(*) rpm, " + countedTokenSumExpression() + " tpm")
...
func SumUsedToken(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string) (token int) {
	tx := LOG_DB.Table("logs").Select(countedTokenSumExpression())
	if username != "" {
